...
import torch
import torch as th
import adarl_envs.experiments.build_adarl_envs_env as build_adarl_envs_env
...
def build_vec_env(env_builder_args, log_folder, seed, num_envs) -> gym.vector.VectorEnv:
    builders = [(lambda i: (lambda: build_adarl_envs_env.env_builder(log_folder=log_folder,
                                                  seed=seed+100000*i,
...
def runFunction(seed, folderName, resumeModelFile, run_id, args):
    ...
    env_builder_args = {
        ...
        "reward_joint_torque_weight" : 0.5,
        "reward_tracking_weight" : 1.0,
        "reward_joint_velocity_weight" : 0.1,
        ...
        "max_steps_per_episode" : int(ep_duration_sec/step_length_sec),
        "obs_only_vec":True,
        "action_smoothing_halflife_sec" : 0.001}
...
def main(seed, folderName, run_id, args, env_builder_args, hyperparams):
    ...
    collector.set_base_collector_model(model_builder=lambda o,a: build_sac(o, a, hyperparams))
    ...
    eval_env, fps = build_adarl_envs_env.env_builder(log_folder=log_folder+"/eval",
                            seed=seed+100000000,
                            env_builder_args = env_builder_args,
                            is_eval=True)
    eval_env_rec = build_adarl_envs_env.wrap_with_recorder(eval_env,
                                                        stepLength_sec=1/fps,
                                                        log_folder=log_folder+"/eval",
                                                        video_save_freq=1)
    eval_env_rec_det = build_adarl_envs_env.wrap_with_recorder(eval_env,
                                                        stepLength_sec=1/fps,
