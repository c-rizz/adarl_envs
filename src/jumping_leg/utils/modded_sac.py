...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        if self.use_sde:
            raise RuntimeError(f"use_sde is not supported")
        ...
        for gradient_step in range(gradient_steps):
            ...
            ent_coef_losses[gradient_step] = ent_coef_loss

            ent_coefs[gradient_step] = ent_coef
        ...
        time_per_iter = (time.monotonic()-t0)/gradient_steps
        with th.no_grad():
            self.logger.record("train/n_updates", self._n_updates, exclude="tensorboard")
            self.logger.record("train/ent_coef", th.mean(ent_coefs).item())
            self.logger.record("train/actor_loss", th.mean(actor_losses).item())
            self.logger.record("train/critic_loss", th.mean(critic_losses).item())
            self.logger.record("train/ent_coef_loss", th.mean(ent_coef_losses).item())
            self.logger.record("train/time_per_iter", time_per_iter)
