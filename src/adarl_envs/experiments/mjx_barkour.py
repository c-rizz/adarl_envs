...
class BarkourEnv(PipelineEnv):
  ...
  def __init__(
    ...
    print(f"opt.integrator = {sys.opt.integrator}")
    print(f"opt.iterations = {sys.opt.iterations}")
    print(f"opt.ls_iterations = {sys.opt.ls_iterations}")
  ...
  def reset(self, rng: jax.Array) -> State:  # pytype: disable=signature-mismatch
    ...
    metrics = {'total_dist': 0.0, 'sim_time' : 0.0, 'stepping_wtime' : 0.0}
  ...
  def step(self, state: State, action: jax.Array) -> State:  # pytype: disable=signature-mismatch
    ...
    state.metrics['sim_time'] += self.dt
    state.metrics['stepping_wtime'] += tf-t0
...
if method == "ppo":
    ...
    train_fn = functools.partial(
        sac.train, num_timesteps=timesteps, num_evals=int(timesteps/eval_freq),
        reward_scaling=1, episode_length=1000, normalize_observations=True,
        action_repeat=1, discounting=0.99, learning_rate=3.0e-3,
        num_envs=5000, batch_size=1024,
        network_factory=make_networks_factory,
        randomization_fn=domain_randomize,
...
runname = f"barkour_{method}_{int(time.time())}"
def progress(num_steps, metrics):
  ...
  dt = 0.02
  print(f"{date} : steps={num_steps} : reward={metrics['eval/episode_reward']} : wHz={num_steps/(t-t0)} : rt={num_steps*dt/(t-t0)}") # : rt={metrics['sim_time']/(t-t0):.2f}")
  print(f"                               : {metrics}")
  # plt.xlim([-train_fn.keywords['num_timesteps']*0.1, train_fn.keywords['num_timesteps'] * 1.1])
  # plt.ylim([0, 2000])
  plt.clf()
...
media.write_video(runname+".mp4",eval_env.render(rollout[::render_every]), fps=1.0 / eval_env.dt / render_every)
